...
== Updates from tpope's original pickler (by glennr)

  - Pickler pull no longer parses the story first. Great for getting stories into tracker first, then you worry about formatting them in Cucumber-perfect fashion later.
  - Story URLs can start with both a '@' and a '#' comment style
  
== Getting started

  gem install glennr-pickler --source=http://gemcutter.org
